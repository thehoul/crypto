...
impl<G: AffineRepr> KBPositiveAccumulator<G> {
    ...
    >(
        ...
        let member = Self::accumulator_member::<D, E>(element, sk);
    ...
    >(
        ...
        let members = cfg_into_iter!(elements)
            .map(|element| Self::accumulator_member::<D, E>(element, sk))
            .collect::<Vec<_>>();
    ...
    pub fn from_accumulated(accumulated: G) -> Self {
...
pub mod tests {
    ...
    fn membership() {
        ...
        test_serialization!(KBPositiveAccumulator<G1Affine>, accumulator);
        ...
        for _ in 0..count {
            ...
            assert!(accumulator
                .get_witness::<Blake2b512, Bls12_381>(&elem, &sk, &params, &state)
                .is_err());

            let accum_member = KBPositiveAccumulator::<G1Affine>::accumulator_member::<
                Blake2b512,
                Bls12_381,
            >(&elem, &sk);
            ...
            let m_wit = accumulator
                .get_witness::<Blake2b512, Bls12_381>(&elem, &sk, &params, &mut state)
                .unwrap();
        ...
        for elem in elems {
            ...
            accumulator = accumulator
                .remove::<Blake2b512, Bls12_381>(&elem, &sk, &mut state)
                .unwrap();
            assert_eq!(
    ...
    fn batch_update_and_membership() {
        ...
        for i in 0..additions.len() {
            ...
            accumulator_1
                .add::<Blake2b512, Bls12_381>(&elem, &sk, &params, &mut state_1)
                .unwrap();
        }
        ...
        let wits = accumulator_2
            .add_batch::<Blake2b512, Bls12_381>(additions.clone(), &sk, &params, &mut state_2)
            .unwrap();
        ...
        for i in 0..removals.len() {
            accumulator_1 = accumulator_1
                .remove::<Blake2b512, Bls12_381>(&removals[i], &sk, &mut state_1)
                .unwrap();
        }
        ...
        accumulator_2 = accumulator_2
            .remove_batch::<Blake2b512, Bls12_381>(&removals, &sk, &mut state_2)
            .unwrap();
        assert_eq!(*accumulator_1.value(), *accumulator_2.value());
        assert_eq!(state_1.db, state_2.db);
        ...
        for e in removals.iter() {
            accumulator_3
                .add::<Blake2b512, Bls12_381>(e, &sk, &params, &mut state_3)
                .unwrap();
        ...
        // Remove a batch
        let upd = accumulator_4
            .batch_updates::<Blake2b512, Bls12_381>(vec![], &removals, &sk, &params, &mut state_4)
            .unwrap();
        accumulator_4 = upd.0;
